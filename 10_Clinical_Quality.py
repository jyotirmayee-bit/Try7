...
Radiology Dashboard
-------------------
UNIT DASHBOARD for the Radiology incharge.
...
st.set_page_config(
    page_title="Srikara Hospitals | Radiology",
    page_icon="🩻",
    layout="wide",
)
...
st.title("🩻 Radiology — Unit Dashboard")
st.caption("Owner: Radiology")

# ---------------------------------------------------------------------
# Load data and keep only this department's rows
# ---------------------------------------------------------------------
master_df = load_master_data()
dept_df = filter_by_department(master_df, "Radiology")
...
with tab_detail:
    ...
    search = st.text_input("Search KPIs in Radiology", placeholder="e.g. mortality, occupancy...")
