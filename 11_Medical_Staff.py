...
--------------------------
UNIT DASHBOARD for the Clinical Quality incharge.
...
st.set_page_config(
    page_title="Srikara Hospitals | Clinical Quality",
    page_icon="✅",
    layout="wide",
)
...
st.title("✅ Clinical Quality — Unit Dashboard")
st.caption("Owner: Quality Head")

# ---------------------------------------------------------------------
# Load data and keep only this department's rows
# ---------------------------------------------------------------------
master_df = load_master_data()
dept_df = filter_by_department(master_df, "Clinical Quality")
...
with tab_detail:
    ...
    search = st.text_input("Search KPIs in Clinical Quality", placeholder="e.g. mortality, occupancy...")
...
with tab_data:
    ...
    st.download_button(
        ...
        data=csv_bytes,
        file_name="clinical_quality.csv",
        mime="text/csv",
    )
