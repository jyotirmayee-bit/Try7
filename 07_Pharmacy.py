...
---------------------------
UNIT DASHBOARD for the Billing & Revenue incharge.
...
st.set_page_config(
    page_title="Srikara Hospitals | Billing & Revenue",
    page_icon="💵",
    layout="wide",
)
...
st.title("💵 Billing & Revenue — Unit Dashboard")
st.caption("Owner: Billing")

# ---------------------------------------------------------------------
# Load data and keep only this department's rows
# ---------------------------------------------------------------------
master_df = load_master_data()
dept_df = filter_by_department(master_df, "Billing & Revenue")
...
with tab_detail:
    ...
    search = st.text_input("Search KPIs in Billing & Revenue", placeholder="e.g. mortality, occupancy...")
...
with tab_data:
    ...
    st.download_button(
        ...
        data=csv_bytes,
        file_name="billing_and_revenue.csv",
        mime="text/csv",
    )
